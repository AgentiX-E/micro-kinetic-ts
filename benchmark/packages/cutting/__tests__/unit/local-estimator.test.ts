...
import type { CuttingWindow, TimeSeries } from '@agentix-e/micro-kinetic-core';
import {
...
describe('computeLinearErrorBound', () => {
  it('computes ε = C * r * δ² / 2', () => {
    expect(computeLinearErrorBound(2, 3)).toBeCloseTo(9);
  });
  it('custom scale', () => {
    expect(computeLinearErrorBound(2, 3, 0.5)).toBeCloseTo(4.5);
  });
  it('throws on NaN', () => {
    expect(() => computeLinearErrorBound(NaN, 1)).toThrow();
  });
});

describe('computeExponentialErrorBound', () => {
  it('computes exponential bound', () => {
    expect(computeExponentialErrorBound(2, 1, 0.5)).toBeGreaterThan(0);
  });
  it('uses Taylor for small x', () => {
    expect(computeExponentialErrorBound(2, 0.001, 0.5)).toBeGreaterThan(0);
  });
});

describe('computePowerLawErrorBound', () => {
  it('computes power-law bound', () => {
    expect(computePowerLawErrorBound(2, 2, 2)).toBeCloseTo(4);
  });
...
describe('computeLogarithmicErrorBound', () => {
  it('computes log bound', () => {
    expect(computeLogarithmicErrorBound(2, 3)).toBeGreaterThan(0);
  });
  it('zero duration', () => {
    expect(computeLogarithmicErrorBound(5, 0)).toBe(0);
  });
});
...
describe('LocalErrorEstimator', () => {
  ...
  describe('detectDegradationType', () => {
    it('returns LINEAR for small data', () => {
      expect(estimator.detectDegradationType(makeTS('mem', [0, 1000, 2000], [100, 200, 300]))).toBe(
    ...
    it('returns valid type for larger data', () => {
      ...
      expect(
        [DegradationType.LINEAR, DegradationType.EXPONENTIAL, DegradationType.POWER_LAW].includes(
          type,
        ),
      ).toBe(true);
    });
    ...
    it('detects exponential for exponential-like data', () => {
      const ts = makeTS('exp', [0, 1000, 2000, 3000, 4000, 5000, 6000], [1, 2, 4, 8, 16, 32, 64]);
      const type = estimator.detectDegradationType(ts);
      expect(type).toBe(DegradationType.EXPONENTIAL);
    });

    it('detects power-law for quadratic data', () => {
      const ts = makeTS('pow', [0, 1000, 2000, 3000, 4000, 5000, 6000], [1, 4, 9, 16, 25, 36, 49]);
      const type = estimator.detectDegradationType(ts);
  ...
  describe('config', () => {
    ...
    it('supports custom config', () => {
      const custom = new LocalErrorEstimator({
        defaultType: DegradationType.EXPONENTIAL,
        scaleFactor: 2.0,
      });
