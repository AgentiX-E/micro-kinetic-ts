...
#: Row-level channel markers: they sit on the service row itself.
ROW_MARKERS: tuple[tuple[str, re.Pattern[str]], ...] = (
    ('onset', re.compile(r'\bonset=')),
    ('latency-edges', re.compile(r'\blatEdges=')),
    ('failed-edge', re.compile(r'\bfailedEdge=')),
    ('dominant-metric', re.compile(r'\bdominant=')),
    ('signature-overlap', re.compile(r'\bboth=')),
)
...
SUB_LINE_MARKERS: tuple[tuple[str, re.Pattern[str]], ...] = (
    ('decisive-composition', re.compile(r'^\s+metricDecisive:')),
)
...
class ChannelCoverage:
    """
    How far one channel reaches, over BOTH populations the artifact has.

    `rows` is `None` for a case-scoped channel, which is not the same as zero rows carrying it: the
    question does not apply, and a reader that conflated the two would report a header field as missing
    from every row of every case.
    """
