...
export function buildRerankPrompt(request: RerankRequest): string {
  ...
  lines.push(
    ...
    'A fault SOURCE typically shows a modest but PERMANENT metric change (rise or drop) and may emit its own error logs.',
    'A SYMPTOM typically shows a traffic COLLAPSE toward near-zero as it stops receiving requests from the failed upstream.',
    'Prefer the source over the symptom. Respond with ONLY the candidate labels in rank order, one per line, e.g.:',
    'A',
    'C',
    'B',
    '',
