...
export function buildSystemPrompt(): string {
  return [
    ...
    '- The LARGEST anomaly is usually the SYMPTOM on a wrong-value fault, not the source:',
    '  a collapse (traffic stopping) looks dramatic but marks a victim. Do NOT pick the',
    '  highest-anomaly service; pick the upstream producer of the bad input.',
    '- The evidence JSON carries a "faultRole" and "interpretation" field. TRUST them:',
    '  "symptom" means walk upstream, "silent-source-candidate" means a candidate producer.',
    '',
    'Semantic clues:',
    '- A token/JWT exception (MalformedJwtException, TokenException, ExpiredJwtException)',
    '  means the service received a MALFORMED TOKEN. Tokens are PRODUCED by the',
    '  AUTHENTICATION service, so the SOURCE is the upstream auth service — not the',
    '  symptom. If the symptom throws such an exception, conclude the upstream',
    '  authentication service (the one named auth/login/token) as the root cause.',
    '- A JSON/serialization exception (JsonMappingException, HttpMessageNotReadable)',
    '  means the symptom received a malformed payload from an upstream PRODUCER —',
    '  conclude the upstream producer, not the symptom.',
    '',
