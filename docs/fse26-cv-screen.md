...
| the INERT/UNEVALUABLE split, and its GOLDEN | `35328237540` at `57d8dac`: **9 of 9 cells byte-identical** — `RE1 80.0 / 92.8 / 68.0`, `RE2 82.4 / 88.9 / 68.1`, `RE3 80.0 / 45.0 / 51.1` — with `CI` and `Release` green on the same commit. Owed by the paths rule (`benchmarks/src/**`) and the expected outcome: the change is report TEXT, so a cell can only move if the split altered a verdict, and the same window table prints either way. |
