...
| the **`earliest-only`** shape of the onset prior (credit ONLY the service(s) that moved first — a MASK on the onset, where the competitor and the credited source are not the same quantity) — **VERIFIED end to end**: the run that shipped it flipped **exactly the four datapacks the solver named in advance** and none other (`broken 0`), and its own dump reads back at `rank-1 1422/1422` with `unexplained 0` | `fse26-onset-verdict.md` §4, §7 | **SHIPPED at `temporalWeight = 0.036552` + `onsetShape = 'earliest-only'`** — the first candidate on this axis to meet its pre-registered bar, and the pair is the measurement: one commit, control **756** (`35021503281`) against candidate **760** (`35021510164`), Top@1 **53.45%** against 53.16%, three types up (`HTTPRequestDelay +2 / JVMMemoryStress +1 / JVMReturn +1`) and **ZERO REGRESSED FAULT TYPES**. The weight is the midpoint of the solved zero-loss window `[0.034920, 0.038183]` — the same solver and the same midpoint rule the pool penalty shipped under, so two terms cannot be described by two rules. **The other half of the kill criterion is the golden 9-cell, and it is measured by the same push**: RCAEval has no dispatch inputs, so the only way to run it at a candidate configuration is to ship the candidate and read the run the push triggers — the revert condition is therefore stated in advance, and a moved cell reverts the WEIGHT while keeping the shape (which is inert at weight 0). Two further measurements make the gain more than a fit: the mask is **sharp** (76.5% of cases have a UNIQUE first mover; mean tied set 1.34) and the **reversed control** (`latest-only`) gains **0**, so the direction is not arbitrary. It is also the one-sided shape, which is why the mechanism that killed its two-sided siblings does not apply to it (`pruner.ts`, `OnsetShape`) | a shape that is neither min-max in the delay, linear in the onset RANK, nor either one-sided mask — or a window with a LARGER gain at zero loss. Do not re-propose the two shapes in the row above: the free read is one command (`--onset-screen`), which is also how any new shape is screened before a run |
