...
| the **`earliest-only`** shape of the onset prior (credit ONLY the service(s) that moved first — a MASK on the onset, where the competitor and the credited source are not the same quantity) | `fse26-onset-verdict.md` §4 | **the first candidate in three iterations to meet the axis's pre-registered bar**: gain **+4, 0 lost** in `[0.034920, 0.038183]`, ship `0.036552`, `HTTPRequestDelay +2 / JVMMemoryStress +1 / JVMReturn +1`. Two measurements make it more than a fit: the mask is **sharp** (76.5% of cases have a UNIQUE first mover; mean tied set 1.34) and the **reversed control** (`latest-only`) gains **0**, so the direction is not arbitrary. Its two edges are one constant apart (`0.010050`-scale lead vs slope gap 1.000000), so the window is thin and named | shipping needs a shape selector PLUS a weight in the ranking path, then the PAIR at the shipped configuration with **zero regressed fault types** and the golden 9-cell byte-identical. Note a shape change is provably inert while `temporalWeight = 0` (the term is multiplied by the weight), so the whole risk is in the weight — measure it, do not argue it |
