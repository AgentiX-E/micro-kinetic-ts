...
| per-edge latency-reward signal (`latWeight`) | `fse26-latency-term-verdict.md` | matched pair at `0.75`: 47.33% → **55.20%**, 673→785 (**+7.88pp**), Top@3 +11.88pp, Top@5 +13.01pp, RCAEval golden 9/9 byte-identical — but **7 regressed fault types** (ReplaceCode −2, ResponseAbort −2, five at −1), so the criterion's second half failed. The weight was then **SOLVED, not swept**: `--window 1 --slope lat` on the full dump gives a zero-regression window of `w ∈ [0, 0.030459]`, bound by one `JVMException` case, and the point **`w = 0.03` was MEASURED**: 47.33% → **48.80%**, 673→694 (**+21**), **zero regressed fault types** — so the axis is no longer closed, it is `SHIPPED` at 0.030. The window's edge is confirmed from both sides: `0.031` measures 696 with exactly one regressed type, `JVMException`, which is the type the reconstruction named as the binder | a weight or variant that keeps **zero** regressed fault types and gains **more than +21** — the frontier is `w ≈ 0.6646` for +118 but costs 11 regressed cases, so a candidate must close that gap on the regression side, not the gain side. The first three candidate mechanisms are measured and negative: `failedEdgeWeight` at any weight (no weight exists), the topological-source gate (rejected by simulation, −71 `ReplaceCode` cases), and every direction/volume/aggregation variant of `failedEdgeMinRecords` |
