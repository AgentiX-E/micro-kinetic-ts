...
| per-edge latency-reward signal (`latWeight`) | `fse26-latency-term-verdict.md` | matched pair at `0.75`: 47.33% → **55.20%**, 673→785 (**+7.88pp**), Top@3 +11.88pp, Top@5 +13.01pp, RCAEval golden 9/9 byte-identical — but **7 regressed fault types** (ReplaceCode −2, ResponseAbort −2, five at −1), so the criterion's second half fails | a weight or variant with **zero** regressed fault types that keeps a gain — **SOLVED**: the zero-regression window is `w ∈ [0, 0.030459]` and `w = 0.0305` predicts **+24** cases; the unconstrained optimum is `w ≈ 0.6646` for +118 at 11 regressed types. Both points are dispatched for measurement; until they land, the shipped default stays 0 |
